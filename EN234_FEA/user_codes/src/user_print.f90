...
!!
  ...
  integer ::  n_state_vars_per_intpt                                         ! No. state variables per integration point
  real (prec) ::   vol_averaged_strain(6)                                    ! Volume averaged strain in an element
  real (prec) ::   vol_averaged_stress(6)                                    ! Volume averaged stress in an element
  real (prec), allocatable ::   vol_averaged_state_variables(:)              ! Volume averaged state variables in an element
  ...
  call compute_element_volume_average_3D(lmn,vol_averaged_strain,vol_averaged_state_variables,length_state_variable_array, &
   ...
   if (n_state_vars_per_intpt<6) then
      write(user_print_units(1),'(2(D12.5))') vol_averaged_strain(1),vol_averaged_stress(1)
...
subroutine compute_element_volume_average_3D(lmn,vol_averaged_strain,vol_averaged_state_vars,length_output_array, &
    ...
    integer, allocatable    :: node_list(:)                                ! List of nodes on the element (coSSectivity)
    ...
    real (prec)  ::  strain(6), dstrain(6), epsdev(6), straintotal(6),zero(6)          ! Strain vector contains [e11, e22, e33, 2e12, 2e13, 2e23]
...
!    real (prec)  :: E, xnu, D44, D11, D12              ! Material properties
    real (prec)  :: volume, straineff, stresseff, Et, Es, eij, el_vol
    real (prec)  :: matrix1(6,6), matrix2(6,6)
    real (prec)  :: sig0, eps0, NN, KK
...
! define deviatoric strain
      epsdev(1) = straintotal(1)-1.d0/3.d0*(straintotal(1)+straintotal(2)+straintotal(3))
      epsdev(2) = straintotal(2)-1.d0/3.d0*(straintotal(1)+straintotal(2)+straintotal(3))
      epsdev(3) = straintotal(3)-1.d0/3.d0*(straintotal(1)+straintotal(2)+straintotal(3))
      epsdev(4) = straintotal(4)/2.d0
      epsdev(5) = straintotal(5)/2.d0
      epsdev(6) = straintotal(6)/2.d0

! define effective strain
      eij = 0
      do i = 1,3
          eij = eij+(epsdev(i)*epsdev(i))
      end do
      do i = 4,6
          eij = eij+2.d0*(epsdev(i)*epsdev(i))
      end do
      straineff = dsqrt(2.d0/3.d0*eij)

! compare effective strain and epsilon zero, calculate effective stress and Es Et
      Et = 0.d0
      Es = 0.d0
      if (straineff <= eps0) then
      stresseff = (dsqrt( (1.d0+NN**2.d0)/(NN-1.d0)**2.d0-(NN/(NN-1.d0)-straineff/eps0)**2.d0)-1.d0/(NN-1.d0))*sig0
      Et = (NN/(NN-1.d0)-straineff/eps0)/eps0*sig0
      Et=Et/dsqrt( (1.d0+NN**2.d0)/(NN-1.d0)**2.d0-(NN/(NN-1.d0)-straineff/eps0)**2.d0)
      else
      stresseff = ((straineff/eps0)**(1.d0/NN))*sig0
      Et = (straineff/eps0)**(1.d0/NN-1.d0)/(NN*eps0)*sig0
      end if


! define matrix in D and e_dyadic_e
      matrix1 = 0.d0
      do i = 1,3
      matrix1(i,i)=2.d0
      end do
      ...
      e_dyadic_e = spread(epsdev,dim=2,ncopies=6)*spread(epsdev,dim=1,ncopies=6)

! calculate stress
      if (straineff > 0.d0) then
         stress(1:3) = 2.d0/3.d0*stresseff*epsdev(1:3)/straineff+KK*(straintotal(1)+straintotal(2)+straintotal(3))
         stress(4:6) = 2.d0/3.d0*stresseff*epsdev(4:6)/straineff
         Es = stresseff/straineff
         D = 4.d0/(9.d0*straineff**2)*(Et-Es)*e_dyadic_e+Es/3.d0*matrix1+(KK-2.d0*Es/9.d0)*matrix2
     else
         stress(1:3) = KK*(straintotal(1)+straintotal(2)+straintotal(3))
         stress(4:6) = 0
         D = Et/3.d0*matrix1+(KK-2.d0*Et/9.d0)*matrix2
      end if
...
!    real (prec)  ::  sigmatrix(2,2)                    ! Defining stress matrix
!    real (prec)  ::  epsmatrix(2,2)                    ! Defining strain matrix
...
!    real (prec)  ::  WW
!
!    integer    :: node_identifier                              ! Flag deltaifying node type
!    integer    :: element_identifier                           ! Flag deltaifying element type (specified in .in file)
!    integer    :: n_nodes                                      ! # nodes on the element
!    integer    :: n_properties                                 ! # properties for the element
!    integer    :: n_state_variables                            ! # state variables for the element
...
!    integer    ::  i, ii , jj, nn             ! Loop counter
!
!
...
!
!! Defining B matrix based on the results of dN/dx
!        B = 0.d0
...
!
!! Calculate the stress and strain based on B and D matrix
!        strain = matmul(B,dof_total)
!        dstrain = matmul(B,dof_increment)
!        strain = strain + dstrain
!        stress = matmul(D,strain)
!
!! Place the calculated stress and strain in sigma and epsilon matrix
!        sigmatrix(1,1) = stress(1)
!        sigmatrix(2,2) = stress(2)
!        sigmatrix(1,2) = stress(3)
!        sigmatrix(2,1) = stress(3)
!        epsmatrix(1,1) = strain(1)
!        epsmatrix(2,2) = strain(2)
!        epsmatrix(1,2) = strain(3)/2.d0
!        epsmatrix(2,1) = strain(3)/2.d0
!        xmap = 0.d0
!
!        do i = 1, n_nodes
!        xmap(1)=xmap(1)+N(i)*x(1,i)
!        xmap(2)=xmap(2)+N(i)*x(2,i)
!        end do
!
